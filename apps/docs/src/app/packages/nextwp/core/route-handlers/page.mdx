...
A single unified route handler that can be used for all NextWP integrations with WordPress such as preview and revalidation. It handles all requests to the `/app/api/wp/[...nextwp]/route.ts` directory.
...
import { unifiedRouteHandler } from '@nextwp/core'

const handler = unifiedRouteHandler({
  preview: {
