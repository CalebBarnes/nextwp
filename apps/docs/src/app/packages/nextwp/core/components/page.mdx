...
A component used in a dynamic route to render Wordpress pages and posts.

### Props

<Properties>
  <Property name="params" type="object">
    The params object from a Next.js dynamic route.
  </Property>
  <Property name="searchParams" type="object">
    The searchParams object from a Next.js dynamic route.
  </Property>
...
<Note>
  Using the CLI tool, you can quickly add the `WordpressTemplate` dynamic route
  and scaffold a starter `templates` directory in an existing project.
</Note>
...
Or, you can add a dynamic route manually
...
export default async function DefaultPageTemplate({ data }: { data: WpPage }) {
  return (
    <FlexibleContent rows={data?.acf?.my_flexible_content} blocks={blocks} />
  )
}
```
