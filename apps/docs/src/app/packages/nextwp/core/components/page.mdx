...
<Note>
  The CLI generator is a planned feature and not yet implemented: Using the [CLI
  tool](/packages/nextwp/cli), you can quickly add the `WordpressTemplate`
  dynamic route and scaffold a starter `templates` directory in an existing
  project.
</Note>
